...
def train_target(args):
    ...
    while iter_num < max_iter:
        ...
        if args.ent:
            ...
            ent_loss_record.append(entropy_loss.item())
            total_loss_record.append(ent_loss_record[-1])
            if args.gent:
                msoftmax = softmax_out.mean(dim=0)
                gentropy_loss = -torch.sum(-msoftmax * torch.log(msoftmax + 1e-5))
                gent_loss_record.append(gentropy_loss.item())
                total_loss_record[-1] += gent_loss_record[-1]
            
            if args.gent:
                im_loss = (entropy_loss + gentropy_loss) * args.ent_par
            else:
